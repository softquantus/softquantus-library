"""
Generate native .qmod files from SoftQLib Jupyter notebooks.

For every call to softqlib.synthesize() encountered while running a notebook,
...
WHAT GETS COLLECTED
  ...
  contain library code, utilities, and test files that are not SoftQLib scripts.
...
OUTPUT
  ...
  that the notebook explicitly overrides.  Fields left at their softqlib SDK
...
PATCH = """\
# ============================================================
# softqlib model-saver patch  (injected by generate_qmod_from_notebooks.py)
# ============================================================
...
import softqlib as _cq
import softqlib.synthesis as _cq_syn
from softqlib import Constraints as _Constraints, Preferences as _Preferences
from softqlib.qmod.write_qmod import write_qmod
...
def _patched_synthesize(model, auto_show=False, constraints=None, preferences=None):
    ...
    if save_dir:
        ...
        # model (via softqlib's set_constraints / set_preferences) so write_qmod
        # sees them.
...
def collect_targets(paths: list[str]) -> list[Path]:
    ...
    for p in paths:
        ...
        if path.is_dir():
            ...
            # utilities, and test files that are not SoftQLib synthesis scripts.
