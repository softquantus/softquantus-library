...
def vaildate_block_encoding(
    ...
    execution_prefs = ExecutionPreferences(
        num_shots=1,
        backend_preferences=SoftQLibBackendPreferences(
            backend_name=SoftQLibSimulatorBackendNames.SIMULATOR_STATEVECTOR
        ),
...
def run_simulation(
    main_func,
    # backend_name=SoftQLibSimulatorBackendNames.SIMULATOR_STATEVECTOR,
    backend_name=SoftQLibNvidiaBackendNames.SIMULATOR_STATEVECTOR,
    block_name: str | None = "block",
    show: bool = False,
) -> tuple[pd.DataFrame, QuantumProgram]:
    from softqlib.interface.generator.model.preferences.preferences import (
        TranspilationOption,
    )
    ...
    execution_prefs = ExecutionPreferences(
        num_shots=1,
        backend_preferences=SoftQLibBackendPreferences(backend_name=backend_name),
...
def dataframe_to_state_vector(
    ...
    """
    Reconstruct the state vector from a SoftQLib execution dataframe.
...
def plot_fields_from_dataframe(
    ...
    """
    Plot Ez, Bx, By from a softqlib execution dataframe whose main function
