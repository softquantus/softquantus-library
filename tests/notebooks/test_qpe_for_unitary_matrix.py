...
def test_notebook(tb: TestbookNotebookClient) -> None:
    ...
    for qprog, e_width, e_depth in zip(
        tb.ref_pydantic("qprogs"), actual_widths, tb.ref("softqlib_depths")
    ):
    ...
    for cx_count_softqlib, cx_count_qiskit in zip(
        tb.ref("softqlib_cx_counts"),
        tb.ref("qiskit_cx_counts"),
    ):
        assert cx_count_softqlib < cx_count_qiskit
