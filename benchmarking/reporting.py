...
def get_softqlib_version() -> str:
    try:
        return version("softqlib")
    except PackageNotFoundError:
        try:
            import softqlib

            return getattr(softqlib, "__version__", "unknown")
        except Exception:
            return "unknown"
...
def write_version_file(root: str | Path = "../report") -> None:
    ...
    softqlib_version = latex_escape(get_softqlib_version())
    version_path.write_text(
        rf"\newcommand{{\softqlibversion}}{{{softqlib_version}}}" + "\n",
        encoding="utf-8",
    )
